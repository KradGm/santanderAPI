        ...
        List<HistoryDto> history)
{
    ...
    public UserDto(User model) {
        this(
                ...
                ofNullable(model.getHistory()).orElse(emptyList()).stream().map(HistoryDto::new).collect(toList())
        );
    }
    ...
    public User toModel() {
        ...
        model.setHistory(ofNullable(this.history).orElse(emptyList()).stream().map(HistoryDto::toModel).collect(toList()));
